...
static __global__ void moe_vec_q(
  ...
  // expert_stride_bytes == 0: dense contiguous banks (original layout). > 0: each
  // expert starts at a fixed byte stride (padded banks for mixed-quant models).
  const block_q_t* x = expert_stride_bytes > 0
...
static void moe_vec_q4_0_q8_1_cuda(
  ...
  moe_vec_q<scalar_t, QK4_0, QI4_0, block_q4_0, VDR_Q4_0_Q8_1_MMVQ, vec_dot_q4_0_q8_1>
      <<<block_nums, block_dims, 0, stream>>>(vx, vy, dst, topk_ids, top_k, ncols, nrows, token_stride, expert_stride_bytes);
}
